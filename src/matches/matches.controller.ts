  ...
  export class MatchesController {
    ...
    async list(@Req() req: any, @Query() _q: any) {
      ...
      if (!meId) {
        ...
        return items.map((m) => {
          ...
          // TEAM_MATCH ise highlightUntil yoksa 36 saatlik default ver
      ...
      /* 3) TEK bir return: access içine isSeriesMember ekle + TEAM_MATCH fallback highlight */
      return normalized.map(m => {
        // TEAM_MATCH olup highlightUntil yoksa: createdAt + 36 saat ver
    ...
    async findOne(@Param('id') id: string, @Req() req: any) {
      ...
      // TEAM_MATCH ise ve highlightUntil boşsa: createdAt + 36 saat
      if (m.createdFrom === 'TEAM_MATCH' && !m.highlightUntil) {
        const untilIso = new Date(new Date(m.createdAt as any).getTime() + 36 * 3600 * 1000).toISOString();
        return { ...base, highlightUntil: untilIso };
      }
      return base;
    }
